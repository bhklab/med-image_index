...
class IndexTCIA:
    ...
    def _group_by_collection(self, series_list: list[dict]) -> dict[str, list[dict]]:
        ...
        grouped: dict[str, list[dict]] = {c: [] for c in self.tcia_collections}
    ...
    def apply_updates(self, update: TCIAUpdate) -> None:
        ...
        self._ensure_collection_dirs()
        for collection, series in update.by_collection.items():
            if series:
                self.process_series(series, collection)
    ...
    def crawl_collections(self) -> None:
        """Run imgtools Crawler on collections that have downloaded images."""
        for collection in self.tcia_collections:
            images_dir = self.output_path / collection / "images"
            if not images_dir.exists() or not any(images_dir.iterdir()):
                continue
            logger.info("Crawling collection %s", collection)
            crawler = Crawler(images_dir, force=True)
            crawler.crawl()
            logger.info(
                "Finished indexing collection %s, output path: %s",
                collection,
                self.output_path / collection,
            )

    def _ensure_collection_dirs(self) -> None:
        """Create `images/` under each TCIA collection output path."""
        for collection in self.tcia_collections:
            (self.output_path / collection / "images").mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    index.summarize_updates("01/05/2026")
